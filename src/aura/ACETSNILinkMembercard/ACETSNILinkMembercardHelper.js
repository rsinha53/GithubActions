({
    ...
    searchResMembers: function (component, event, helper) {
        ...
        var action = component.get('c.findMembersFromCDB');
        ...
        var woId = component.get("v.workorderids");
        ...
        action.setCallback(this, function (response) {
            ...
            if (state == 'SUCCESS') {
                ...
                //console.log('result--ACETSNILinkMembercarHelper-->'+JSON.stringify(result));
                ...
                console.log('code ---- ' + JSON.stringify(result) + result.statusCode );
                var multipleEntries = false;
                if(!result.hasOwnProperty('multlpleEntriesFound')) {
                    multipleEntries = false;
                }else{
                    multipleEntries = result.multlpleEntriesFound;
                } 
                if(multipleEntries==true){
                    component.set('v.columns1', [
                        {label: 'Last Name', fieldName: 'familyName', type: 'text'},
                        {label: 'First Name', fieldName: 'givenName', type: 'text'},
                        {label: 'Date of Birth', fieldName: 'birthDate', type: 'text'},
                        {label: 'Policy Number', fieldName: 'policyNumber', type: 'text'},
                    ]);
                        let lst = result.recordsFoundInCDB;
                        var dataLst = [] ;
                                  for (let i = 0; i < lst.length; i++) {
                        console.log('check --- ' + JSON.stringify(lst[i]));
                        var ob = lst[i];
                        if (! $A.util.isEmpty(ob.identifiers.identity_enterpriseId)) {
                            ob.enterpriseID = ob.identifiers.identity_enterpriseId[0].enterpriseID;
                        }
                        ob.IsSandbox = result.IsSandbox;
                        ob.mapNameToWebservice = result.mapNameToWebservice;
                        ob.token = result.token;
                        ob.woId = result.woId;
                        ob.referralIdentifier = result.referralIdentifier;
                        ob.CHNEligible = result.CHNEligible;
                        if (! $A.util.isEmpty(ob.identifiers.payer_memberId)) {
                            ob.policyNumber = ob.identifiers.payer_memberId[0].policyNumber;    
                        }                   
                        dataLst.push(ob);
                    }
                    component.set("v.data1", dataLst);
                    component.set("v.showDataTableForMultipleEntries", true);
                }
                ...
                if(result.statusCode == 200){ 
                    ...
                    console.log('eligible and qualification' + result.CHNEligible + result.CHNQualified);
                    
                    if (! $A.util.isEmpty(result.houseHoldResultWrapper.memberDetails)) {
                        
                        let acetMemResult = result.houseHoldResultWrapper.memberDetails.newMemberMatch;
                        console.log('acetMemResult--'+JSON.stringify(acetMemResult));
                        //debugger;
                        if (! $A.util.isEmpty(acetMemResult)){
                            for(var key in acetMemResult) {
                                var rows =acetMemResult[key];
                                console.log('rows--'+rows);
                                var rmap = result.houseHoldResultWrapper.memberDetails.nameRelationMap;
                                console.log('rmap--'+JSON.stringify(rmap));
                                var nameSplit =key.toString().split('~');
                                console.log('nameSplit--'+rmap[key]);
                                var keyName = '';
                                var sliceName = '';
                            ...
                            component.set('v.addnewUser',addnewUser); 
                            result.houseHoldResultWrapper.memberDetails.newMemberMatch = newMemExistingDetails; 
                            console.log('addnewUser--'+JSON.stringify(addnewUser));
                            
                            component.set("v.newMemberMatch",newMemExistingDetails);
                            result.newAddressMatch = newMemExistingDetails;   
                            console.log('newMemExistingDetails---'+JSON.stringify(newMemExistingDetails));
                            
                            component.set("v.memberDetails",result.houseHoldResultWrapper.memberDetails);
                            if( result.houseHoldResultWrapper.memberDetails.isAcetSearch == 'SearchACET')
                                component.set("v.showMemDetailsModalSecnd",true);
                            else	component.set("v.showMemDetailsModal", true);
                    ...
                    helper.callHouseHoldWS(component, event, helper);*/
                            //commented the above code to replace it with CDB code - by Cherry
                            
                        } else {
                            console.log('inside else---291');
                            if ((result.CHNEligible == false || result.CHNEligible == undefined) && (result.CHNQualified == false || result.CHNQualified == undefined) && ($A.util.isEmpty(result.houseHoldResultWrapper.memberDetails.accountId))) {
                                component.set("v.showRefferal", true);
                                helper.hideGlobalSpinner(component);
                            }
                            else{
                                console.log('inside else--297');
                                var accountId = result.houseHoldResultWrapper.memberDetails.accountId;
                                console.log('accountId-'+accountId);
                                
                                //code to redirect to the account detail page
                                if(accountId  != null || accountId != undefined){
                                    ...
                                    helper.fireToast('Something went wrong');
                                }
                                helper.hideGlobalSpinner(component);                     
                            }
                        }
                    ...
                    else if (result.statusCode == 404 || (result.statusCode == 404 && (result.CHNEligible == false || result.CHNEligible == undefined) && (result.CHNQualified == false || result.CHNQualified == undefined) && result.message!=null && result.message == "Eligibilities Not Found.")) {
                        component.set("v.showRefferal", true);
                        helper.hideGlobalSpinner(component);
                    }else {
                        //var responseMsg = result.message;
                        //var jsonString = JSON.parse(responseMsg);
                        console.log('last else---359');
    ...
    callHouseHoldWS : function(component,event,helper) {
        ...
        if(transactionId){
            ...
            action.setCallback(this, function(response) {
                ...
                if(state == 'SUCCESS') {
                    var result = response.getReturnValue();
                    console.log('Link extended:'+JSON.stringify(result));
                    ...
                    if(result.statusCode == 200){
                        ...
                        //Oxford logic -Begin
                        var isOxford = component.get("v.Oxford_isOxford");
                        if(isOxford){
                            helper.familyInfo(component, event, helper);
                        }
                        //end
                        else{
                            component.set('v.showServiceErrors', true);
    ...
    //Method added for Oxford -Begin
    familyInfo: function (component, event, helper) {
        console.log('getFamilyInfo-------------');
        var getFamilyInfoAction = component.get("c.getFamilyInfo");
        var memberId=component.get("v.memId");
        //var policyId=component.get("v.Oxford_groupNumber");
        var surrogateKey=component.get("v.Oxford_surrogateKey");
        var groupNumber=component.get("v.Oxford_groupNumber");
        var effectiveDate=component.get("v.Oxford_effectiveDate");
        var benefitPlanId=component.get("v.Oxford_benefitPlanId");
        var bundleOptionId=component.get("v.Oxford_benefitBundleOptionId");
        getFamilyInfoAction.setParams({
            "memberId": memberId,
        ...
        getFamilyInfoAction.setCallback(this, function(res) {
            if(res.getState() == 'SUCCESS') {
                var res = res.getReturnValue();
                console.log('getFamilyInfoAction response-------------');
                console.log(res);
                console.log('statusCode='+res.statusCode);
                if(res.statusCode == 200){
                    helper.checkUhgAccess(component, event, helper,res);
                }
                else{
                    component.set('v.showServiceErrors', true);
                    if (component.get("v.showHideMemAdvSearch") == true && component.get("v.displayMNFFlag") == true) {
                    ...
                    component.set('v.serviceMessage', res.message);
                    helper.fireToast(res.message);
                    helper.hideGlobalSpinner(component);
        ...
        $A.enqueueAction(getFamilyInfoAction);
    },
    //end
    checkUhgAccess: function (component, event, helper, result) {
        ...
        for(var i=0; i< result.houseHoldResultWrapper.houseHoldList.length;i++){
            if(result.houseHoldResultWrapper.houseHoldList[i].isMainMember == true) {
                ...
                sourceCode = result.houseHoldResultWrapper.houseHoldList[i].sourceCode;
                cosmosDiv = result.houseHoldResultWrapper.houseHoldList[i].cosmosDivision;
            }
        }
        ...
        checkUhgAction.setCallback(this, function(response1) {
            ...
            if(state1 == 'SUCCESS') {
                ...
                var memberCard = component.get("v.memberCardData");
                var grpNum = memberCard.CoverageLines[0].GroupNumber;
                if(resultVal){
                    //var uhgAcc =  result1.
                    helper.checkOnshoreRestriction(component, event, helper, grpNum, sourceCode, cosmosDiv, plcId, result);
                }
                else{
                    component.set('v.showServiceErrors', true);
                    if (component.get("v.showHideMemAdvSearch") == true && component.get("v.displayMNFFlag") == true) {
    ...
    checkOnshoreRestriction: function (component, event, helper, grpNum, sourceCode, cosmosDiv, plcId, result) {
        var isAllowedAccess = true;
        var checkOnshoreRest = component.get("c.checkOnshoreRestriction");
        checkOnshoreRest.setParams({"sourceCode" : sourceCode,
                                    "policyNumber" : plcId,
                                    "groupNumber" : grpNum,
                                    "cosmosDivision" : cosmosDiv});
        checkOnshoreRest.setCallback(this, function(response1) {
            var state1 = response1.getState();
            if(state1 == 'SUCCESS') {
                isAllowedAccess = response1.getReturnValue();
                if(!isAllowedAccess){
                    component.set('v.showServiceErrors', true);
                    if (component.get("v.showHideMemAdvSearch") == true && component.get("v.displayMNFFlag") == true) {
                        component.set("v.mnf", 'mnf');
                    ...
                    component.set('v.serviceMessage', 'You do not have access to view Onshore Restricted policy members');
                    helper.fireToast('You do not have access to view Onshore Restricted policy members');
                    helper.hideGlobalSpinner(component); 
                } else {
                    helper.sniEligibilityCall(component, event, helper, result);
                }
        ...
        $A.enqueueAction(checkOnshoreRest);
    },
    sniEligibilityCall: function (component, event, helper, result) {
        ...
        var householdlist;
        if(result.houseHoldResultWrapper.houseHoldList != undefined ){
            householdlist = result.houseHoldResultWrapper.houseHoldList.length;
        }else{
            householdlist = 0;
        }
        var plcId = '';
        ...
        if(plcId && memberIdVal && memberDOBVal && memberFNVal && memberLNVal)	{
            component.set("v.polId",plcId);
            component.set("v.membId",memberIdVal);
            var policyIdVar;
            ...
            action1.setCallback(this, function(response1) {
                ...
                if(state1 == 'SUCCESS') {
                    ...
                    if(result1.statusCode == 200){
                        ...
                        var lob = result1.SNIresultWrapper.lob;
                        var productTypes = result1.SNIresultWrapper.productTypes;
                        var serviceGroup =result1.SNIresultWrapper.serviceGroup;
                        console.log('lob----------'+lob);
                        console.log('productTypes----------'+productTypes);
                        console.log('serviceGroup----------'+serviceGroup);
                        //End
                        console.log('assignedToVal----------'+assignedToVal);
                        console.log('sniEligible@@@@='+sniEligible);
                        console.log('Workorder Id_Ankit____'+ component.get("v.workorderids"));
                        var workorderId = component.get("v.workorderids");
                        var memberCard = component.get("v.memberCardData");
                        var groupNumber = memberCard.CoverageLines[0].GroupNumber;
                        //if(sniEligible != null && sniEligible != '' && typeof sniEligible != 'undefined' && sniEligible != 'not eligible' ){ //add one more condition here
                        if(sniEligible){
                            houseDetail.createAccount(result.houseHoldResultWrapper.houseHoldList,memberDOBVal,workorderId,memberIdVal,advFullName,sniEligible,policyIdVar,plcId,assignedToVal,lob,productTypes,serviceGroup,groupNumber);
                        }
                        ...
                        if(sniEligible && sniEligible=='not eligible'){
                            ...
                            component.set("v.productTypes",productTypes);
                            component.set("v.serviceGroup",serviceGroup);
